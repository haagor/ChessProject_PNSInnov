...
public class SelectController : MonoBehaviour, IObserver, IObservable {
    ...
    /// </summary>
    public void OnNotify(IList<Event> events)
    {
        bool notifiedBefore = false;
        foreach(Event ev in events)
        {
            if (ev.Type == EventType.MovementDone)
            {
                notifiedBefore = true;
            }
        }
    ...
    {
        NotifyAll(new List<Event>());    
    }

    public void NotifyAll(IList<Event> events)
    {
        events.Add(new Event(EventType.MovementDone, "movement done"));
